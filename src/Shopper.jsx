...
{slideItems.map((item)=>{
    ...
    return(
          <img src={product.image} data-id={item.id}className='absolute object-cover w-[220px] h-[300px]' key={item.id} alt=""  style={{ 
      ...
      <div className="relative w-50 h-50">
        ...
        <div className="absolute inset-0 rounded-full bg-gradient-to-br from-gray-200 to-gray-300 shadow-2xl">
          ...
          <div className="absolute inset-8 rounded-full bg-white shadow-inner"></div>
        </div>

        {/* Previous Button (Left) */}
        <button
       onClick={goPrev}
          className="absolute left-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-gray-300 hover:bg-gray-400 active:bg-gray-500 transition-all shadow-md flex items-center justify-center group"
          aria-label="Previous"
        >
          <svg 
        ...
        <button
         onClick={goNext}
          className="absolute right-4 top-1/2 -translate-y-1/2 w-10 h-10 rounded-full bg-gray-300 hover:bg-gray-400 active:bg-gray-500 transition-all shadow-md flex items-center justify-center group"
          aria-label="Next"
        >
          <svg 
            className="w-6 h-6 text-gray-700 group-hover:text-gray-900 transition-colors" 
        ...
        <button
          ...
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-20 h-20 rounded-full bg-gradient-to-br from-gray-400 to-gray-500 hover:from-gray-500 hover:to-gray-600 active:from-gray-600 active:to-gray-700 transition-all shadow-lg flex items-center justify-center group"
          aria-label="Select"
        >
